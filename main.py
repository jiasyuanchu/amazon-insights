...
def track_single_product(asin: str):
    ...
    if success:
        ...
        if "error" not in summary:
            ...
            print(f"Price: ${summary['current_price']}" if summary['current_price'] else "Price: N/A")
            print(f"Rating: {summary['current_rating']}/5" if summary['current_rating'] else "Rating: N/A")
            print(f"Reviews: {summary['current_review_count']}" if summary['current_review_count'] else "Reviews: N/A")
...
def track_all_products():
    ...
    for summary in summaries:
        if "error" not in summary:
            ...
            price = f"${summary['current_price']}" if summary['current_price'] else "N/A"
            rating = f"{summary['current_rating']}/5" if summary['current_rating'] else "N/A"
            print(f"{summary['asin']} | {title:50} | {price:10} | {rating:8} | {summary['price_trend']:10}")
...
def main():
    ...
    parser.add_argument('command', choices=[
        'track-single', 'track-all', 'monitor', 'alerts', 'history', 'test'
