...
def demo_competitive_workflow():
    ...
    try:
        ...
        print(
            f"   - LLMReporter: {'OpenAI enabled' if llm_reporter.use_openai else 'Fallback mode'}"
        )
        ...
        print(
            f"Price Position: {price_analysis.get('price_position', 'unknown').title()}"
        )

        market_range = price_analysis.get("market_price_range", {})
        if market_range:
            print(
                f"Market Range: ${market_range.get('min', 0):.2f} - ${market_range.get('max', 0):.2f}"
            )
            print(f"Market Average: ${market_range.get('average', 0):.2f}")

        print(
            f"Price Advantage: {'Yes' if price_analysis.get('price_advantage') else 'No'}"
        )
        print(f"Cheaper Competitors: {price_analysis.get('cheaper_competitors', 0)}")
        print(
            f"More Expensive Competitors: {price_analysis.get('more_expensive_competitors', 0)}"
        )
        ...
        if rating_stats:
            print(
                f"Market Rating Range: {rating_stats.get('min', 0):.1f} - {rating_stats.get('max', 0):.1f}"
            )
            print(f"Average Market Rating: {rating_stats.get('average', 0):.1f}")
            print(
                f"Rating Position: {rating_stats.get('main_product_position', 'unknown').title()}"
            )

        print(
            f"Quality Advantage: {'Yes' if rating_analysis.get('quality_advantage') else 'No'}"
        )
        print(
            f"Popularity Advantage: {'Yes' if rating_analysis.get('popularity_advantage') else 'No'}"
        )

        print_subsection("BSR Analysis")
        if bsr_analysis and not bsr_analysis.get("error"):
            for category, data in list(bsr_analysis.items())[
                :3
            ]:  # Show top 3 categories
                if isinstance(data, dict):
                    print(f"{category}:")
                    print(
                        f"  Main Product Rank: #{data.get('main_product_rank', 'N/A')}"
                    )
        ...
        if diversity:
            print(
                f"Feature Count: {diversity.get('main_product_features', 0)} vs Avg {diversity.get('average_competitor_features', 0)}"
            )
            print(
                f"Feature Richness: {diversity.get('feature_richness', 'unknown').title()}"
            )
        ...
        if scores:
            ...
            print(
                f"Price Competitiveness: {scores.get('price_competitiveness', 'N/A'):.1f}/100"
            )
            print(
                f"Quality Competitiveness: {scores.get('quality_competitiveness', 'N/A'):.1f}/100"
            )
            print(
                f"Popularity Competitiveness: {scores.get('popularity_competitiveness', 'N/A'):.1f}/100"
            )

        position_summary = competitive_summary.get("position_summary", {})
        if position_summary:
            print(
                f"Overall Position: {position_summary.get('overall_position', 'unknown').title()}"
            )
            print(
                f"Analysis Confidence: {competitive_summary.get('analysis_confidence', 'unknown').title()}"
            )
        ...
        if "error" in positioning_report:
            ...
            print(
                exec_summary[:200] + "..." if len(exec_summary) > 200 else exec_summary
            )
        ...
        print(
            f"   - Generated {'LLM-powered' if not positioning_report.get('error') else 'structured'} positioning report"
        )
...
def show_api_examples():
    ...
    examples = {
        ...
        "System Summary": {"method": "GET", "endpoint": "/api/v1/competitive/summary"},
    }
