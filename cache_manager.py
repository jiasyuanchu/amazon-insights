...
def show_cache_keys():
    ...
    key_categories = {
        "Product Summaries": [
            CacheKeyBuilder.product_summary(asin) for asin in AMAZON_ASINS[:3]
        ],
        ...
        "Alerts": [
            ...
            CacheKeyBuilder.alerts_summary(48),
        ],
    }
    ...
    for category, keys in key_categories.items():
        ...
        for key in keys:
            ...
            ttl_info = (
                f"(TTL: {ttl}s)" if ttl > 0 else f"(TTL: {ttl})" if ttl == -1 else ""
            )
...
def clear_product_cache(asin: str):
    ...
    if keys_cleared:
        print(
            f"✅ Cleared {len(keys_cleared)} cache entries: {', '.join(keys_cleared)}"
        )
    else:
