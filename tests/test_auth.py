...
class TestAPIKeyManager:
    ...
    def test_validate_api_key_cached(self, key_manager):
        ...
        key_manager.redis.get.return_value = key_manager._serialize_cache_data(
            cached_data
        )
...
class TestAuthenticationService:
    ...
    async def test_authenticate_request_valid(self, auth_service):
        ...
        with patch("src.auth.authentication.API_KEY_REQUIRED", True):
            ...
            auth_service.key_manager.validate_api_key.assert_called_once_with(
                "sk_live_testkey"
            )
...
class TestRateLimitConfig:
    ...
    def test_tier_limits_progression(self):
        ...
        assert (
            enterprise_limits[RateLimitWindow.MINUTE]
            > pro_limits[RateLimitWindow.MINUTE]
        )
        assert (
            enterprise_limits[RateLimitWindow.HOUR] > pro_limits[RateLimitWindow.HOUR]
        )
    ...
    def test_endpoint_weights(self):
        ...
        assert (
            config.ENDPOINT_WEIGHTS["/api/v1/products/track"]
            > config.ENDPOINT_WEIGHTS["/api/v1/products/summary"]
        )
        assert (
            config.ENDPOINT_WEIGHTS["/api/v1/competitive/analyze"]
            > config.ENDPOINT_WEIGHTS["/api/v1/products/track"]
        )
...
class TestIntegration:
    ...
    async def test_auth_and_rate_limit_flow(self):
        ...
        with patch("src.auth.authentication.redis_client"), patch(
            "src.auth.rate_limiter.redis"
        ):
