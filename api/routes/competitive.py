    ...
    try:
        ...
        return {
            "summary": {
                ...
                "success_rate": (
                    f"{(successful / len(results) * 100):.1f}%" if results else "0%"
                ),
            },
            "results": results,
        }
...
async def competitive_analysis_summary():
    ...
    try:
        ...
        return {
            ...
            "competitors": {"total_tracked": total_competitors},
            "tracking_status": tracking_summary,
            "system_health": {
                "tracking_coverage": (
                    f"{(tracking_summary['tracked'] / (tracking_summary['tracked'] + tracking_summary['not_tracked']) * 100):.1f}%"
                    if (tracking_summary["tracked"] + tracking_summary["not_tracked"])
                    > 0
                    else "0%"
                ),
                "recommendations": [],
