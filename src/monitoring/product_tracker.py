...
class ProductTracker:
    ...
    def track_all_products(self) -> Dict[str, bool]:
        ...
        logger.info(
            f"Batch tracking completed: {successful}/{len(self.asins)} successful"
        )
    ...
    def _check_for_anomalies(self, asin: str, current_data: Dict):
        try:
            ...
            if current_data.get("price") and previous_snapshot.price:
                price_change = (
                    abs(current_data["price"] - previous_snapshot.price)
                    / previous_snapshot.price
                )
                if price_change > 0.1:  # 10% change
                    logger.warning(
                        f"Significant price change for {asin}: {previous_snapshot.price} -> {current_data['price']}"
                    )
    ...
    def continuous_monitoring(self):
        logger.info(
            f"Starting continuous monitoring with {MONITORING_INTERVAL} hour intervals"
        )
        ...
        while True:
            try:
                ...
                logger.info(
                    f"Monitoring cycle completed in {duration}. Next cycle in {MONITORING_INTERVAL} hours."
                )
