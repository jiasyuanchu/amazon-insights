...
try:
    ...
    print("Warning: OpenAI package not installed. LLM features will use fallback analysis.")
...
class LLMReporter:
    ...
    def _generate_openai_report(self, analysis_data: Dict) -> Dict:
        ...
        try:
            ...
            response = client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert e-commerce competitive analyst. Analyze the provided Amazon product data and generate comprehensive business insights."},
            ...
            report = {
                "executive_summary": self._extract_executive_summary(llm_analysis, competitive_summary, main_product),
                "competitive_positioning": self._analyze_competitive_positioning(price_analysis, rating_analysis, bsr_analysis),
                "strengths_weaknesses": self._extract_swot_analysis(llm_analysis, analysis_data),
                "feature_differentiation": self._analyze_feature_differentiation(feature_analysis),
                "strategic_recommendations": self._extract_recommendations(llm_analysis, analysis_data),
                "market_insights": self._extract_market_insights(llm_analysis, analysis_data),
                "llm_detailed_analysis": llm_analysis,
                "report_metadata": {
                    "generated_at": datetime.now().isoformat(),
                    "model_used": self.model,
                    "llm_enabled": True,
                    "analysis_confidence": competitive_summary.get("analysis_confidence", "medium"),
    ...
    def _generate_fallback_report(self, analysis_data: Dict) -> Dict:
        ...
        report = {
            "executive_summary": self._generate_executive_summary(competitive_summary, main_product),
            "competitive_positioning": self._analyze_competitive_positioning(price_analysis, rating_analysis, bsr_analysis),
    ...
    def _extract_executive_summary(self, llm_analysis: str, competitive_summary: Dict, main_product: Dict) -> str:
    ...
    def _extract_recommendations(self, llm_analysis: str, analysis_data: Dict) -> List[Dict]:
    ...
    def _extract_market_insights(self, llm_analysis: str, analysis_data: Dict) -> Dict:
        ...
        try:
            insights = {"trends": [], "market_dynamics": {}, "competitive_landscape": {}}
            ...
            for line in lines:
                line = line.strip()
                if 'market' in line.lower() and ('insight' in line.lower() or 'trend' in line.lower()):
                    in_insights = True
                    continue
                elif in_insights and line and (line.startswith('-') or line.startswith('•')):
    ...
    def _generate_executive_summary(self, competitive_summary: Dict, main_product: Dict) -> str:
    ...
    def _analyze_competitive_positioning(self, price_analysis: Dict, 
                                       rating_analysis: Dict, bsr_analysis: Dict) -> Dict:
        ...
        positioning = {
            ...
            "quality_positioning": {
                ...
                "position": rating_analysis.get("rating_statistics", {}).get("main_product_position", "unknown"),
        ...
        if price_analysis.get("price_position") == "highest":
            positioning["recommendations"].append("Consider price optimization to improve competitiveness")
        
        if rating_analysis.get("quality_advantage") == False:
            positioning["recommendations"].append("Focus on product quality improvements and customer satisfaction")
    ...
    def _identify_strengths_weaknesses(self, analysis_data: Dict) -> Dict:
        ...
        if any(features for features in missing_features.values()):
            opportunities.append("Feature gaps present opportunities for product enhancement")
    ...
    def _analyze_feature_differentiation(self, feature_analysis: Dict) -> Dict:
        ...
        try:
            ...
            if total_unique + total_common + total_missing > 0:
                differentiation_score = (total_unique * 2 + total_common - total_missing) / (total_unique + total_common + total_missing) * 50
            ...
            return {
                ...
                "differentiation_level": "high" if differentiation_score > 70 else "moderate" if differentiation_score > 40 else "low",
    ...
    def _generate_recommendations(self, analysis_data: Dict) -> List[Dict]:
        ...
        try:
            ...
            overall_score = competitive_summary.get("competitive_scores", {}).get("overall_competitiveness", 0)
            if overall_score < 50:
                recommendations.append({
                    "category": "strategy",
                    "priority": "high",
        ...
        try:
            ...
            if market_range:
                ...
                insights["market_dynamics"] = {
                    "price_volatility": "high" if price_spread > avg_price * 0.5 else "moderate" if price_spread > avg_price * 0.2 else "low",
                    "market_maturity": "fragmented" if price_spread > avg_price * 0.3 else "consolidated",
                    "price_competition_intensity": "high" if len(analysis_data.get("competitors", [])) > 5 else "moderate"
                }
            ...
            insights["competitive_landscape"] = {
                ...
                "quality_differentiation": scores.get("quality_competitiveness", 0) > 80,
                "price_leadership": scores.get("price_competitiveness", 0) > 80,
                "overall_market_position": competitive_summary.get("position_summary", {}).get("overall_position", "unknown")
            }
