...
class CompetitiveManager:
    ...
    def create_competitive_group(
        self, name: str, main_product_asin: str, description: str = None
    ) -> CompetitiveGroup:
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                if main_product_asin not in AMAZON_ASINS:
                    logger.warning(
                        f"Main product {main_product_asin} not in tracking list"
                    )
                ...
                logger.info(
                    f"Created competitive group: {name} with main product {main_product_asin}"
                )
                return group
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                group = (
                    ...
                    .filter(CompetitiveGroup.id == group_id)
                    .first()
                )
                ...
                if existing:
                    logger.warning(
                        f"Competitor {competitor_asin} already exists in group {group_id}"
                    )
                    return existing
    ...
    def get_all_competitive_groups(self) -> List[CompetitiveGroup]:
        ...
        try:
            with self.db_manager.get_session() as session:
                groups = (
                    session.query(CompetitiveGroup)
                    .filter(CompetitiveGroup.is_active == True)
                    .order_by(CompetitiveGroup.updated_at.desc())
                    .all()
                )
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                if competitor:
                    ...
                    logger.info(
                        f"Removed competitor {competitor_asin} from group {group_id}"
                    )
                    return True
                else:
                    logger.warning(
                        f"Competitor {competitor_asin} not found in group {group_id}"
                    )
                    return False
    ...
    def ensure_competitors_tracked(self, group_id: int) -> Dict[str, bool]:
        ...
        try:
            ...
            logger.error(
                f"Error checking tracking status for group {group_id}: {str(e)}"
            )
            return {}
