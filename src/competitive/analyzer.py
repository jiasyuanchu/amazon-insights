...
class CompetitiveAnalyzer:
    ...
    def analyze_competitive_group(self, group_id: int) -> Dict:
        ...
        try:
            ...
            if not main_product_data:
                return {
                    "error": f"Main product data not available for {group.main_product_asin}"
                }
            ...
            analysis_result = {
                ...
                "competitors": [
                    self._metrics_to_dict(comp) for comp in competitors_data
                ],
                "price_analysis": self._analyze_price_positioning(
                    main_product_data, competitors_data
                ),
                "bsr_analysis": self._analyze_bsr_positioning(
                    main_product_data, competitors_data
                ),
                "rating_analysis": self._analyze_rating_positioning(
                    main_product_data, competitors_data
                ),
                "feature_analysis": self._analyze_feature_comparison(
                    main_product_data, competitors_data
                ),
            ...
            logger.error(
                f"Error in competitive analysis for group {group_id}: {str(e)}"
            )
            return {"error": str(e)}
    ...
    def _analyze_price_positioning(
        self, main_product: CompetitiveMetrics, competitors: List[CompetitiveMetrics]
    ) -> Dict:
        ...
        try:
                    ...
                    competitor_prices.append(
                        {
                            ...
                            "price": comp.price,
                            "price_difference": (
                                comp.price - main_price if main_price else None
                            ),
                            "price_difference_percent": (
                                ((comp.price - main_price) / main_price * 100)
                                if main_price
                                else None
                            ),
                        }
            ...
            return {
                ...
                "cheaper_competitors": (
                    len([p for p in competitor_prices if p["price"] < main_price])
                    if main_price
                    else 0
                ),
                "more_expensive_competitors": (
                    len([p for p in competitor_prices if p["price"] > main_price])
                    if main_price
                    else 0
                ),
            ...
            for category, main_rank in main_product.bsr_data.items():
                        ...
                        category_analysis["competitors"].append(
                            {
                                ...
                                "better_rank": comp_rank
                                < main_rank,  # Lower rank is better
                            }
                        )
                ...
                if competitor_ranks:
                    ...
                    category_analysis["rank_statistics"] = {
                        ...
                        "competitors_with_better_rank": len(
                            [r for r in competitor_ranks if r < main_rank]
                        ),
                        "competitors_with_worse_rank": len(
                            [r for r in competitor_ranks if r > main_rank]
                        ),
                    }
    ...
    def _analyze_rating_positioning(
        self, main_product: CompetitiveMetrics, competitors: List[CompetitiveMetrics]
    ) -> Dict:
        ...
        try:
            ...
            for comp in competitors:
                ...
                if comp.rating:
                    ...
                    comp_data["rating_difference"] = (
                        comp.rating - main_rating if main_rating else None
                    )

                if comp.review_count:
                    all_review_counts.append(comp.review_count)
                    comp_data["review_count_difference"] = (
                        comp.review_count - main_reviews if main_reviews else None
                    )
            ...
            if all_ratings:
                rating_stats = {
                    ...
                    "main_product_position": (
                        "highest"
                        if main_rating == max(all_ratings)
                        else "lowest" if main_rating == min(all_ratings) else "middle"
                    ),
                }
            ...
            if all_review_counts:
                review_stats = {
                    ...
                    "main_product_position": (
                        "highest"
                        if main_reviews == max(all_review_counts)
                        else (
                            "lowest"
                            if main_reviews == min(all_review_counts)
                            else "middle"
                        )
                    ),
                }
            ...
            return {
                "main_product": {"rating": main_rating, "review_count": main_reviews},
                "competitors": competitor_ratings,
                "rating_statistics": rating_stats,
                "review_statistics": review_stats,
                "quality_advantage": (
                    main_rating >= statistics.mean(all_ratings)
                    if all_ratings and main_rating
                    else None
                ),
                "popularity_advantage": (
                    main_reviews >= statistics.mean(all_review_counts)
                    if all_review_counts and main_reviews
                    else None
                ),
            }
            ...
            for category, features in main_features.items():
                ...
                for feature in features:
                    if feature not in feature_frequency[category]:
                        feature_frequency[category][feature] = {
                            "main": True,
                            "competitors": [],
                        }
            ...
            for comp in competitors:
                ...
                for category, features in comp_features.items():
                    ...
                    for feature in features:
                        ...
                        feature_frequency[category][feature]["competitors"].append(
                            comp.asin
                        )
            ...
            for category, features in feature_frequency.items():
                ...
                for feature, presence in features.items():
                    ...
                    if presence["main"] and competitor_count == 0:
                        ...
                        competitor_count >= len(competitors) * 0.7
                    ):  # 70% of competitors have it
    ...
    def _calculate_feature_diversity(
        ...
        try:
            ...
            main_feature_count = sum(
                len(features) for features in main_product.key_features.values()
            )

            competitor_feature_counts = []
            for comp in competitors:
                comp_count = sum(
                    len(features) for features in comp.key_features.values()
                )
                competitor_feature_counts.append(comp_count)

            avg_competitor_features = (
                statistics.mean(competitor_feature_counts)
                if competitor_feature_counts
                else 0
            )
            ...
            return {
                ...
                "feature_richness": (
                    "above_average"
                    if main_feature_count > avg_competitor_features
                    else "below_average"
                ),
                "feature_count_comparison": main_feature_count
                - avg_competitor_features,
            }
        ...
        try:
            ...
            if main_product.price:
                ...
                if competitor_prices:
                    ...
                    scores["price_competitiveness"] = max(
                        0, min(100, (2 - price_ratio) * 50)
                    )
            ...
            if main_product.rating:
                ...
                if competitor_ratings:
                    ...
                    max_rating = 5.0
                    scores["quality_competitiveness"] = (
                        main_product.rating / max_rating
                    ) * 100
            ...
            if main_product.bsr_data:
                ...
                main_best_rank = (
                    min(main_product.bsr_data.values())
                    if main_product.bsr_data
                    else float("inf")
                )
                ...
                if competitor_best_ranks:
                    ...
                    if main_best_rank < avg_comp_rank:
                        scores["popularity_competitiveness"] = min(
                            100, (avg_comp_rank - main_best_rank) / avg_comp_rank * 100
                        )
                    else:
                        scores["popularity_competitiveness"] = max(
                            0,
                            50 - (main_best_rank - avg_comp_rank) / avg_comp_rank * 50,
                        )

            # Overall competitiveness (average of all scores)
            if scores:
                scores["overall_competitiveness"] = round(
                    statistics.mean(scores.values()), 1
                )
            ...
            position_summary = {
                "price_position": (
                    "competitive"
                    if scores.get("price_competitiveness", 0) > 50
                    else "expensive"
                ),
                "quality_position": (
                    "superior"
                    if scores.get("quality_competitiveness", 0) > 70
                    else "average"
                ),
                "popularity_position": (
                    "leading"
                    if scores.get("popularity_competitiveness", 0) > 60
                    else "following"
                ),
                "overall_position": (
                    "strong"
                    if scores.get("overall_competitiveness", 0) > 60
                    else "weak"
                ),
            }
    ...
    def get_trend_analysis(self, group_id: int, days: int = 30) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Generating trend analysis for group {group_id} over {days} days"
            )

            return {
                "group_id": group_id,
                "trend_period_days": days,
                "trends": {"price_trends": {}, "bsr_trends": {}, "rating_trends": {}},
