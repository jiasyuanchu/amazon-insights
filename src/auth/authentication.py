...
class APIKey:
    ...
    def __init__(self, key_id: str, key_type: KeyType, permissions: List[Permission], 
                 tier: RateLimitTier, metadata: Dict[str, Any] = None):
        self.key_id = key_id
...
class APIKeyManager:
    ...
    def generate_api_key(self, key_type: KeyType, name: str = None, 
                        custom_permissions: List[Permission] = None) -> str:
    ...
    def _store_api_key(self, api_key: APIKey):
        ...
        self.redis.setex(cache_key, self.cache_ttl, 
                        self._serialize_cache_data(cache_data))
    ...
    def check_permission(self, api_key: APIKey, required_permission: Permission) -> bool:
...
class AuthenticationService:
    ...
    async def authenticate_request(self, 
                                 ...
                                 request: Request = None) -> Optional[APIKey]:
    ...
    def _log_api_usage(self, api_key: APIKey, request: Request):
        ...
        self.key_manager.redis.lpush(usage_key, 
                                    self.key_manager._serialize_cache_data(usage_data))
...
async def get_current_api_key(api_key: APIKey = Depends(auth_service.authenticate_request)) -> APIKey:
