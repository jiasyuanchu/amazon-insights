...
class RateLimiter:
    ...
    async def check_rate_limit(
        self, api_key: APIKey, endpoint: str, request: Request
    ) -> Dict[str, any]:
    ...
    def _check_window_limit(
        self, key_id: str, window: RateLimitWindow, limit: int, weight: float
    ) -> Tuple[int, int]:
...
async def check_rate_limit(
    ...
    request.state.rate_limit_headers = {
        ...
        "X-RateLimit-Remaining": str(
            rate_limit_info["limits"]["minute"] - rate_limit_info["usage"]["minute"]
        ),
        "X-RateLimit-Reset": str(rate_limit_info["reset_times"]["minute"]),
        "X-RateLimit-Tier": api_key.tier.value,
    }
