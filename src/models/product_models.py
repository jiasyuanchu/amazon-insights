...
try:
    from src.models.competitive_models import (
        CompetitiveGroup,
        Competitor,
        ProductFeatures,
...
class PriceAlert(Base):
    ...
    )  # 'price_increase', 'price_decrease', 'bsr_change', etc.
    ...
    def get_latest_snapshot(self, asin: str) -> ProductSnapshot:
        with self.get_session() as session:
            return (
                ...
                .filter(ProductSnapshot.asin == asin)
    ...
    def get_recent_alerts(self, hours: int = 24):
        ...
        with self.get_session() as session:
            ...
            return (
                ...
                .filter(PriceAlert.triggered_at >= cutoff_time)
