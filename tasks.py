...
def track_single_product(self, asin: str) -> Dict:
    ...
    try:
        ...
        if success:
            ...
            logger.info(f"Successfully tracked product {asin}: {summary.get('title', 'N/A')[:50]}...")
...
def track_all_products(self) -> Dict:
    ...
    try:
        ...
        logger.info(f"Batch tracking completed: {successful}/{len(AMAZON_ASINS)} successful")
...
def daily_monitoring() -> Dict:
    ...
    try:
        ...
        # Log summary
        logger.info(f"Daily monitoring completed: {tracking_data['successful']}/{tracking_data['total_products']} products tracked")
        logger.info(f"Total alerts in last 24h: {alerts_summary.get('total_alerts', 0)}")
...
def cleanup_old_data(days_to_keep: int = 90) -> Dict:
    ...
    try:
        ...
        with db_manager.get_session() as session:
            ...
            old_snapshots = session.query(ProductSnapshot).filter(
                ProductSnapshot.scraped_at < cutoff_date
            ).delete()
            
            # Delete old alerts
            old_alerts = session.query(PriceAlert).filter(
                PriceAlert.triggered_at < cutoff_date
            ).delete()
            
            session.commit()
        
        logger.info(f"Cleanup completed: removed {old_snapshots} snapshots, {old_alerts} alerts")
...
def cleanup_expired_cache() -> Dict:
    ...
    try:
        ...
        logger.info(f"Cache cleanup completed. Cache health: {cache_health.get('connected', False)}")
...
def process_anomaly_alerts(self, asin: str, anomalies: List[Dict]) -> Dict:
    ...
    try:
        ...
        for anomaly in anomalies:
            logger.warning(f"Anomaly detected for {asin}: {anomaly.get('message', 'Unknown')}")
